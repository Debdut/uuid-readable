...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.uuidReadable = {}, global.UUID));
  ...
  var generate = function (uuid) {
      if (!uuid) {
          var _uuid = [];
          UUID__namespace.v4({}, _uuid);
          return Readable.generate(_uuid);
      }
      return Readable.generate(UUID__namespace.parse(uuid));
  };
  // Get uuid back from readable.
  var inverse = function (readable) {
      return UUID__namespace.stringify(Readable.inverse(readable));
  };
  // Create a low entropy 32 bit uuid readable.
  var short = function (uuid) {
      return LowEntropy.generate(UUID__namespace.parse(uuid));
  };
  // Check if the low entropy uuid is generated from the given uuid.
  var check = function (readable, uuid) {
      return LowEntropy.check(readable, UUID__namespace.parse(uuid));
  };
